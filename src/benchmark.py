    ...
    for data_folder in data_folders:
        ...
        if speech_file_type == 'auto':
            ...
            for supported_speech_file_type in supported_speech_file_types:
                potential_speech_filepaths = sorted(
                    glob.glob(os.path.join(data_folder, '*.{0}'.format(supported_speech_file_type))))
        ...
        if settings.getboolean('general', 'transcribe'):
            ...
            for speech_file_number, speech_filepath in enumerate(speech_filepaths):
                ...
                for asr_system in asr_systems:
                    transcription, transcription_skipped = transcribe.transcribe(speech_filepath, asr_system, settings,
        ...
        if settings.getboolean('general', 'evaluate_transcriptions'):
            ...
            for asr_system in asr_systems:
                ...
                all_predicted_transcription_file = codecs.open(all_predicted_transcription_filepath, 'w',
                                                               settings.get('general',
                                                                            'predicted_transcription_encoding'))
                all_gold_transcription_filepath = codecs.open(all_gold_transcription_filepath, 'w',
                ...
                for speech_filepath in speech_filepaths:
                    ...
                    if not os.path.isfile(predicted_transcription_txt_filepath):
                        ...
                        predicted_transcription = codecs.open(predicted_transcription_txt_filepath, 'r',
                                                              settings.get('general',
                                                                           'predicted_transcription_encoding')).read().strip()
                ...
                print(
                    ...
                    format(wer * 100, number_of_edits['deletions'], number_of_edits['insertions'],
                           number_of_edits['substitutions'], number_of_tokens_in_gold, asr_system))
                print('Number of speech files: {0}'.format(len(speech_filepaths)))
                print('Number of missing predicted prescription files: {0}'.format(
                    number_of_missing_predicted_transcription_txt_files))
                print('Number of empty predicted prescription files: {0}'.format(
