...
def normalize_tokens(text):
    def other_conditions(pre_token, token, post_token):
        check = True
        if token == "năm" and (post_token.isdigit() or pre_token.isdigit()):
            check = False
        return check
...
if __name__ == "__main__":
    ...
    print(normalize_golden_text(" = không có tiền sử ung thư vú của bản thân và gia đình,  mãn kinh 10 năm , không điều trị hoocmon nội tiết."))
    # print('#' * 100)
